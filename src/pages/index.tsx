...
const PushChatMessageForm: FC<React.HTMLAttributes<HTMLFormElement>> = ({
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      if (isProcessing) {
        alert("Wait for Travitar 9000 to answer");
      }
        ...
        <li key={index} className="flex w-full flex-row items-end gap-x-2">
          {message.role === "assistant" && (
            <div className="object-fit relative block h-10 w-10 flex-none overflow-hidden rounded-full bg-neutral-200">
              <Image
                src={
                  "https://i.ibb.co/JpmwxJd/new-1.jpg"
                }
                alt="Assistant"
                fill
                priority
...
const Home: NextPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Travitar 9000</title>
        <meta name="description" content="Hi, I am Travitar 9000. Let's chat." />
      </Head>
      <main className="mx-auto flex h-screen max-w-3xl flex-col bg-white p-5 md:px-0">
        <div className="flex-none">
          ...
          <p className="text-sm text-neutral-800 sm:text-base">
            This is Travitar 9000, a simple, yet elegant chat bot, from a more civilized age, that you can talk to find
